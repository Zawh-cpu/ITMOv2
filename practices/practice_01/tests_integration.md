...
| I2. Тело без поля `diff` | `KeyError` даёт необработанную ошибку | `POST /api/reviews` с `{}` (`curl -s -o /dev/null -w "%{http_code}" -X POST http://<host>:<port>/api/reviews -H "Content-Type: application/json" -d '{}'`) | Ожидание: в diff нет обработки `KeyError`, ответ 500 это поведение FastAPI по умолчанию (общее знание, проверить запуском). TO BE: контролируемая ошибка 4xx (допущение) | TestClient с `raise_server_exceptions=False` или curl; сравнить код |
...
| I4. Сбой LLM внутри запроса | Исключение доходит до клиента без контролируемого ответа | fake бросает `RuntimeError`, `POST` с `{"diff": "x"}` | Ожидание: в diff нет обработки исключения, ответ 500 (поведение FastAPI по умолчанию, проверить запуском). TO BE: контролируемая ошибка (допущение) | TestClient с `raise_server_exceptions=False` |
